...
def generate_hunyuan3d_model(
    ...
    Parameters:
    - text_prompt: (Optional) A description of the desired model (max 1024 chars).
                   Note: Only works in Professional mode. Rapid mode only supports image input.
    - input_image_url: (Optional) The local or remote url of the input image.

    Returns:
    - When successful, returns a JSON with job_id indicating the task is in progress
    - When the job completes, the status will change to "DONE"
    - Returns error message if the operation fails

    Note: Use Professional mode for text-to-3D, Rapid mode for faster image-to-3D.
    """
    try:
        blender = get_blender_connection()
        ...
        4. Hunyuan3D
            ...
            If Hunyuan3D is enabled:
                if Hunyuan3D mode is "OFFICIAL_API":
                    - Professional (PRO) mode: Supports both text-to-3D and image-to-3D, 3 concurrent tasks
                    - Rapid mode: Image-to-3D only, faster generation, 1 concurrent task
                    - For objects/models, do the following steps:
                        1. Create the model generation task
                            - Use generate_hunyuan3d_model
                            - For text-to-3D: Must use Professional mode
                            - For image-to-3D: Can use either Professional or Rapid mode
                            - Go to cloud.tencent.com to get their own SecretId and SecretKey
                        2. Poll the status
